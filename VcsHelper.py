...
class VcsHelper(object):
	...
	def vcs_file_path(cls, view, vcs_path):
		"""Returns the relative path to the file in the Sublime Text
		view, in the repository rooted at vcs_path."""
		if not vcs_path:
			return False
		full_file_path = os.path.realpath(view.file_name())
		vcs_path_to_file = full_file_path.replace(vcs_path, '').replace('\\', '/')
	...
	def vcs_root(cls, directory):
		...
		if os.path.exists(os.path.join(directory, cls.meta_data_directory())):
			return directory
		else:
	...
	def vcs_tree(cls, view):
		...
		the file in the passed Sublime Text view."""
